<script setup>
import { getCurrentInstance, onMounted } from 'vue'
import GlobalMenuItem from './GlobalMenuItem.vue'
import store from '@/services/store'

defineOptions({
  name: 'GlobalMenu',
  __isStatic: true
})

const currentInstance = getCurrentInstance()
const $props = defineProps(['data'])
const $emit = defineEmits(['action'])

const methods = {
  show (value = true) {
    currentInstance.vnode.el.classList.toggle('app-global-menu--active', value)
    store.set('menuShow', value)
  },
  setActiveClass (el) {
    currentInstance.vnode.el.querySelectorAll('.app-global-menu__hover').
        forEach(i => i.classList.remove('app-global-menu__hover'))

    while (true) {
      el = el?.closest('li')

      if (!el) {
        break
      }

      el.classList.add('app-global-menu__hover')
      el = el.parentElement
    }
  },
  loadData (url, p) {
    url ??= p.data['url']
    p.data['loading'] = true
    p.data['data'] = []

    const params = {}

    if (p.filter !== undefined) {
      params.filter = p.filter
    }

    axios.get(url, {
      params
    }).then(({ data }) => {
      const prepend = []
      const pagination = data['meta']['pagination'] ?? {}

      if (data['meta']['prepend']) {
        prepend.push(...data['meta']['prepend'])
      }

      if (pagination?.['total'] > pagination?.['per'] || p.filter) {
        prepend.push({ filter: p.filter || '' })
      }

      p.data['data'] = [].concat(prepend, (data['data'] || []).map(i => {
        i.to = {
          path: data['meta']['route'].replace(':id', i.id)
        }

        return i
      }))

      if (pagination?.['prev'] || pagination?.['next']) {
        p.data['data'].push({
          total: pagination['total'] ?? null,
          info: pagination['info'] ?? null,
          prev: pagination['prev'] ?? null,
          next: pagination['next'] ?? null
        })
      }

    }).finally(() => {
      p.data['loading'] = false
    })
  }
}

const action = (...args) => {
  if (typeof methods[args[0]] === 'function') {
    methods[args[0]](...Array.from(args).splice(1))
  } else {
    $emit('action', ...args)
  }
}

onMounted(() => {
  methods.show(false)

  document.addEventListener('click', event => {
    const target = event.target.closest('li')

    if (target && target.closest('.app-global-menu')) {
      if (target.classList.contains('app-global-menu__parent')) {
        methods.show(!currentInstance.vnode.el.classList.contains('app-global-menu--active'))
      } else {
        methods.show(false)
      }
    } else if (currentInstance.vnode.el.classList.contains('app-global-menu--active')) {
      methods.show(false)
    }
  })
})
</script>

<template>
  <div class="app-global-menu">
    <ul>
      <global-menu-item v-for="(i, k) in $props.data" :data="i" :key="k" @action="action"/>
    </ul>
  </div>
</template>

<style>
.app-global-menu {
  @apply relative bg-gray-700 text-gray-100
}
.app-global-menu.app-global-menu--active {
  @apply z-40
}
.app-global-menu::before {
  @apply fixed left-0 top-14 right-0 bottom-0 bg-black/5 dark:bg-black/20 opacity-0 invisible transition-all;
  content: "";
}
.app-global-menu.app-global-menu--active::before {
  @apply opacity-100 visible
}
.app-global-menu > ul {
  @apply lg:justify-between px-1
}
.app-global-menu ul, .app-global-menu li {
  @apply flex lg:grow cursor-default
}
.app-global-menu > ul > li li {
  @apply flex items-center justify-between transition
}
.app-global-menu > ul > li {
  @apply grow lg:grow-0 my-1 rounded
}
.app-global-menu > ul > li > ul, .app-global-menu > ul > li > ul > li {
  @apply grow
}
.app-global-menu > ul > li > ul > li > * {
  @apply rounded
}
.app-global-menu > ul > li li.app-global-menu__hover > a, .app-global-menu > ul > li > ul > li.app-global-menu__hover > span {
  @apply bg-gray-600 dark:bg-gray-600 cursor-pointer
}
.app-global-menu > ul > li > ul > li > a, .app-global-menu > ul > li > ul > li > span {
  @apply py-3 px-5
}
.app-global-menu li > a, .app-global-menu li > span {
  @apply flex justify-center items-center py-2 px-5 w-full h-full
}
.app-global-menu > ul > li > ul > li li.app-global-menu__hover > a {
  @apply bg-blue-600 text-white dark:bg-blue-600
}
.app-global-menu > ul > li > ul > li li.app-global-menu__hover > span.disabled {
  @apply bg-inherit text-inherit cursor-default
}
.app-global-menu li .app-global-menu__icon {
  @apply w-5 text-center
}
.app-global-menu > ul > li > ul > li > * > .app-global-menu__icon {
  @apply text-lg leading-[0]
}
.app-global-menu li img {
  @apply -my-1 -mx-1.5 min-w-[2rem] max-w-none max-h-none h-8 w-auto block relative
}
.app-global-menu li .app-global-menu__title {
  @apply grow truncate
}
.app-global-menu > ul > li > ul > li > a .app-global-menu__title, .app-global-menu > ul > li > ul > li > span .app-global-menu__title {
  @apply hidden lg:inline-block
}
.app-global-menu li .app-global-menu__icon + .app-global-menu__title {
  @apply ml-3
}
.app-global-menu li .app-global-menu__locked {
  @apply text-rose-500 text-sm leading-[0] ml-2
}
.app-global-menu li .app-global-menu__disabled > .app-global-menu__title{
  @apply text-rose-600 dark:text-rose-500
}
.app-global-menu li .app-global-menu__id {
  @apply ml-2 text-sm opacity-70
}
.app-global-menu > ul > li ul > li > span .app-global-menu__toggle, .app-global-menu > ul > li > ul > li > a .app-global-menu__toggle {
  @apply hidden lg:inline-flex
}
.app-global-menu li .app-global-menu__toggle {
  @apply -mr-3 -my-2 w-10 h-10 inline-flex items-center justify-center opacity-70
}
.app-global-menu li .app-global-menu__toggle i {
  @apply pointer-events-none transition
}
.app-global-menu.app-global-menu--active > ul > li > ul > li.app-global-menu__hover > * > .app-global-menu__toggle i {
  @apply rotate-180
}
.app-global-menu > ul > li li li .app-global-menu__toggle i {
  @apply -rotate-90
}
.app-global-menu.app-global-menu--active > ul > li > ul > li > .app-global-menu__loading .app-global-menu__toggle {
  @apply inline-flex -ml-2 lg:inline-flex lg:ml-0
}
.app-global-menu.app-global-menu--active > ul > li > ul > li > .app-global-menu__loading .app-global-menu__icon {
  @apply hidden lg:inline-flex
}
.app-global-menu li .app-global-menu__pagination {
  @apply -bottom-1 -mb-1 mt-auto sticky bg-inherit
}
.app-global-menu li .app-global-menu__pagination > span {
  @apply flex grow items-center px-3 py-2 text-gray-500 dark:text-gray-100 text-sm select-none
}
.app-global-menu li .app-global-menu__pagination .app-global-menu__prev, .app-global-menu li .app-global-menu__pagination .app-global-menu__next {
  @apply flex items-center justify-center w-6 h-6 rounded cursor-pointer border-none text-blue-500 hover:bg-gray-100 dark:hover:bg-white/10
}
.app-global-menu li .app-global-menu__pagination .app-global-menu__prev[disabled], .app-global-menu li .app-global-menu__pagination .app-global-menu__next[disabled] {
  @apply opacity-20 pointer-events-none
}
.app-global-menu li .app-global-menu__pagination span {
  @apply grow text-center
}
.app-global-menu li .app-global-menu__filter {
  @apply relative w-full px-3 py-1
}
.app-global-menu li .app-global-menu__filter input {
  @apply pl-2 pr-6 py-0.5 mt-[1px]
}
.app-global-menu li .app-global-menu__filter .app-global-menu__clear {
  @apply text-rose-500 cursor-pointer absolute z-10 top-4 right-6
}
.app-global-menu > ul > li > ul > li.app-global-menu__parent {
  @apply lg:relative
}
.app-global-menu > ul > li > ul > li.app-global-menu__parent > ul {
  @apply mt-1
}
.app-global-menu > ul > li > ul li.app-global-menu__parent > ul {
  @apply absolute opacity-0 invisible top-full left-1.5 right-1.5 lg:right-auto lg:left-0 flex-col lg:min-w-full lg:w-[20rem] rounded py-1 shadow-2xl bg-white text-gray-950 dark:bg-gray-700 dark:text-gray-50 transition;
  max-height: calc(100vh - 4rem);
}
.app-global-menu > ul > li > ul > li ul ul {
  @apply overflow-auto overflow-x-hidden
}
.app-global-menu > ul > li:last-of-type li.app-global-menu__parent > ul {
  @apply lg:left-auto lg:right-0
}
.app-global-menu > ul > li > ul li li.app-global-menu__parent > ul {
  @apply lg:left-full top-0 left-0 right-0
}
.app-global-menu.app-global-menu--active li.app-global-menu__parent.app-global-menu__hover > ul {
  @apply opacity-100 visible z-10 max-h-[calc(100vh_-_4rem)]
}
.app-global-menu.app-global-menu--active > ul > li li.app-global-menu__parent.app-global-menu__hover > ul {
  @apply h-[calc(100vh_-_4rem)] lg:h-auto
}
.app-global-menu .app-global-menu__back {
  @apply -top-1 -mt-1 mb-1 sticky z-10 bg-inherit hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer text-blue-500
}
</style>
