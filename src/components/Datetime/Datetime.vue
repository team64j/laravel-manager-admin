...
function cx (a) {
  return {
    ...
    input () {
      ...
      if ($props.inputClass) {
        c.push($props.inputClass)
      }

      if ($props.error) {
        c.push('!border-rose-500 focus:ring-rose-500')
      }

      if (data.loading) {
        c.push('!text-transparent')
      }
  ...
  <div class="w-full" :class="cx('el')">
    ...
    <div class="relative">
      ...
      <input
          ...
          @blur="onClose">
      <i v-if="$props.clear"
         class="fa fa-calendar-xmark absolute block right-0 top-0 m-3 cursor-pointer text-gray-300 dark:text-gray-500 hover:text-rose-500 dark:hover:text-rose-600 transition"
         @click="onClear"/>
    </div>
